...
Then('I expect the barometric pressure for each day to be displayed', (table) => {
    expect(weatherForecastPage.pressureDailySummary()).to.equal(table.raw().toString())
});
...
Then('I expect (only ){int} three hour periods to be visible', (periods) => {
    expect(weatherForecastPage.threeHourPeriodsVisible()).to.equal(periods)
});

Then('I expect for {string} these three hour periods will be displayed', (day, table) => {
    expect(weatherForecastPage.threeHourPeriodsList(day)).to.equal(table.raw().toString())
});

Then('I expect {string} to display a {string} temperature of {string} degrees', (day, limit, tempValue) => {
    expect(weatherForecastPage.temperatureDailySummary(day, limit)).to.equal(tempValue)
});

Then('I expect for {string} the rainfall total of {string} will be displayed', (day, rainfall) => {
    expect(weatherForecastPage.rainfallDailySummary(day)).to.equal(rainfall)
});

Then('I expect the current/dominant condition of {string} to be displayed in {string} description', (summary, day) => {
   expect(weatherForecastPage.descriptionDailySummary(day, summary)).to.be.true
});

Then('I expect the current/highest wind speed {string} to be displayed for {string} summary', (speed, day) => {
    expect(weatherForecastPage.windSpeedDailySummary(day)).to.equal(speed)
});
